...
describe('StatefulRuleEngine', () => {
  ...
  describe('Constructor', () => {
    ...
    it('should initialize empty state storage', async () => {
    ...
    it('should register state operators with base engine', async () => {
  ...
  describe('Basic Evaluation', () => {
    ...
    it('should evaluate first rule with no previous state', async () => {
      const context = { user: { status: 'pending' } };
      const result = await statefulEngine.evaluate('test-rule', rule, context);
    ...
    it('should detect state changes', async () => {
      ...
      const result1 = await statefulEngine.evaluate('test-rule', rule, context1);
      expect(result1.success).toBe(false);
      expect(result1.triggered).toBe(false);

      // Second evaluation - status changed
      const result2 = await statefulEngine.evaluate('test-rule', rule, context2);
      expect(result2.success).toBe(true);
      expect(result2.triggered).toBe(true);
      expect(result2.stateChange).toBe('triggered');
    });

    it('should maintain state between evaluations', async () => {
      const context1 = { user: { status: 'pending' } };
      const context2 = { user: { status: 'active' } };
      const context3 = { user: { status: 'active' } }; // Same as context2

      await statefulEngine.evaluate('test-rule', rule, context1);
      await statefulEngine.evaluate('test-rule', rule, context2);
      const result3 = await statefulEngine.evaluate('test-rule', rule, context3);
    ...
    it('should handle multiple rules independently', async () => {
      ...
      await statefulEngine.evaluate('rule1', rule1, context1);
      await statefulEngine.evaluate('rule2', rule2, context1);

      const result1 = await statefulEngine.evaluate('rule1', rule1, context2);
      const result2 = await statefulEngine.evaluate('rule2', rule2, context2);
  ...
  describe('Triggering Logic', () => {
    ...
    it('should follow standard triggering behavior by default', async () => {
      const context1 = { user: { status: 'pending' } };
      const context2 = { user: { status: 'active' } };
      const context3 = { user: { status: 'completed' } };

      // First: false, no trigger
      const r1 = await statefulEngine.evaluate('test', rule, context1);
      expect(r1.triggered).toBe(false);

      // Second: true, triggers (false → true)
      const r2 = await statefulEngine.evaluate('test', rule, context2);
      expect(r2.triggered).toBe(true);

      // Third: true, no trigger (true → true)
      const r3 = await statefulEngine.evaluate('test', rule, context3);
      expect(r3.triggered).toBe(false);
    });

    it('should trigger on first successful evaluation', async () => {
      const context = { user: { status: 'active' } };
      const result = await statefulEngine.evaluate('test', rule, context);

      expect(result.success).toBe(false); // No previous state
      expect(result.triggered).toBe(false); // No trigger on first eval
    });

    it('should handle triggerOnEveryChange option', async () => {
      const customEngine = new StatefulRuleEngine(baseEngine, {
        triggerOnEveryChange: true,
      });
      ...
      await customEngine.evaluate('test', rule, context1);
      const r2 = await customEngine.evaluate('test', rule, context2);
      const r3 = await customEngine.evaluate('test', rule, context3);

      expect(r2.triggered).toBe(true); // Change detected
      expect(r3.triggered).toBe(true); // Another change detected
    });

    it('should handle per-evaluation triggerOnEveryChange override', async () => {
      const context1 = { user: { status: 'pending' } };
      const context2 = { user: { status: 'active' } };
      const context3 = { user: { status: 'completed' } };

      await statefulEngine.evaluate('test', rule, context1);
      await statefulEngine.evaluate('test', rule, context2);

      // Override for this evaluation
      const result = await statefulEngine.evaluate('test', rule, context3, {
        triggerOnEveryChange: true,
      });
  ...
  describe('State Change Detection', () => {
    it('should detect initial state', async () => {
      const result = await statefulEngine.evaluate('test', { changed: ['value'] }, { value: 1 });
      expect(result.stateChange).toBe('initial');
    });

    it('should detect triggered state', async () => {
    ...
    it('should detect untriggered state', async () => {
      const rule = { changed: ['value'] };
      await statefulEngine.evaluate('test', rule, { value: 1 });
      await statefulEngine.evaluate('test', rule, { value: 2 }); // triggered
      const result = await statefulEngine.evaluate('test', rule, { value: 2 }); // same
      expect(result.stateChange).toBe('untriggered');
    });

    it('should detect maintained-true state', async () => {
      const rule = { gt: ['value', 5] }; // Non-change operator
      await statefulEngine.evaluate('test', rule, { value: 10 });
      const result = await statefulEngine.evaluate('test', rule, { value: 8 });
      expect(result.stateChange).toBe('maintained-true');
    });

    it('should detect maintained-false state', async () => {
      const rule = { gt: ['value', 10] };
      await statefulEngine.evaluate('test', rule, { value: 5 });
      const result = await statefulEngine.evaluate('test', rule, { value: 3 });
  ...
  describe('Event System', () => {
    ...
    it('should fire evaluated event on every evaluation', async () => {
      const rule = { changed: ['value'] };
      await statefulEngine.evaluate('test', rule, { value: 1 });
    ...
    it('should fire triggered event on state change to true', async () => {
      const rule = { changed: ['value'] };
      await statefulEngine.evaluate('test', rule, { value: 1 });
      await statefulEngine.evaluate('test', rule, { value: 2 });

      expect(eventData.triggered).toBeDefined();
      expect(eventData.triggered.result.success).toBe(true);
    });

    it('should fire untriggered event on state change to false', async () => {
      const rule = { changed: ['value'] };
      await statefulEngine.evaluate('test', rule, { value: 1 });
      await statefulEngine.evaluate('test', rule, { value: 2 }); // triggered
      await statefulEngine.evaluate('test', rule, { value: 2 }); // untriggered

      expect(eventData.untriggered).toBeDefined();
      expect(eventData.untriggered.result.success).toBe(false);
    });

    it('should fire changed event on any state change', async () => {
      const rule = { changed: ['value'] };
      await statefulEngine.evaluate('test', rule, { value: 1 });
      await statefulEngine.evaluate('test', rule, { value: 2 });
    ...
    it('should support adding and removing listeners', async () => {
      const callback = jest.fn();

      statefulEngine.on('evaluated', callback);
      await statefulEngine.evaluate('test', { changed: ['value'] }, { value: 1 });
      expect(callback).toHaveBeenCalledTimes(1);

      statefulEngine.off('evaluated', callback);
      await statefulEngine.evaluate('test', { changed: ['value'] }, { value: 2 });
      expect(callback).toHaveBeenCalledTimes(1); // No additional calls
    });
  });

  describe('History Management', () => {
    it('should not store history by default', async () => {
      await statefulEngine.evaluate('test', { changed: ['value'] }, { value: 1 });
      expect(statefulEngine.historyManager.size()).toBe(0);
    });

    it('should store history when enabled', async () => {
    ...
    it('should get history for specific rule', async () => {
      const historyEngine = new StatefulRuleEngine(baseEngine, {
        storeHistory: true,
      });
  ...
  describe('State Clearing', () => {
    beforeEach(async () => {
      // Set up some state
      await statefulEngine.evaluate('rule1', { changed: ['value'] }, { value: 1 });
      await statefulEngine.evaluate('rule2', { changed: ['value'] }, { value: 2 });
    });

    it('should clear specific rule state', async () => {
    ...
    it('should clear all state when no ruleId provided', async () => {
      const historyEngine = new StatefulRuleEngine(baseEngine, {
        storeHistory: true,
      });
      historyEngine.evaluate('rule1', { changed: ['value'] }, { value: 1 });
  ...
  describe('Batch Evaluation', () => {
    it('should evaluate multiple rules with same context', async () => {
      const rules = {
      ...
      const batch = await statefulEngine.evaluateBatch(rules, context);
    ...
    it('should maintain independent state for each rule', async () => {
      const rules = {
        rule1: { changed: ['value'] },
        rule2: { changed: ['value'] },
      };
      ...
      await statefulEngine.evaluateBatch(rules, { value: 1 });

      // Second batch - both should detect change
      const batch = await statefulEngine.evaluateBatch(rules, { value: 2 });
    ...
    it('should support batch evaluation options', async () => {
      const rules = {
        rule1: { changed: ['value'] },
      };
      ...
      await statefulEngine.evaluateBatch(rules, { value: 1 });
      await statefulEngine.evaluateBatch(rules, { value: 2 }); // triggered

      // Third evaluation with triggerOnEveryChange
      const batch = await statefulEngine.evaluateBatch(
        rules,
        { value: 3 },
        {
          triggerOnEveryChange: true,
    ...
    describe('Error Handling (Phase 2)', () => {
      it('should handle errors gracefully and continue by default', async () => {
        ...
        const batch = await statefulEngine.evaluateBatch(rules, {
          status: 'active',
          score: 75,
        });
      ...
      it('should collect error details when enabled', async () => {
        const rules = {
          errorRule: { gt: 'malformed' }, // Malformed rule causes error
        };

        const batch = await statefulEngine.evaluateBatch(rules, { value: 1 });
      ...
      it('should include error metadata in results', async () => {
        const rules = {
          errorRule: { between: 'not-array' }, // Malformed
        };

        const batch = await statefulEngine.evaluateBatch(rules, { value: 5 });
      ...
      it('should handle all rules failing', async () => {
        const rules = {
          bad1: { gt: 'bad' },
          bad2: { lt: 'bad' },
          bad3: { between: 'bad' },
      ...
      it('should handle empty rules object', async () => {
        const batch = await statefulEngine.evaluateBatch({}, { value: 1 });
  ...
  describe('Integration with Base Engine', () => {
    it('should work with existing operators', async () => {
      const mixedRule = {
      ...
      await statefulEngine.evaluate('mixed', mixedRule, context1);
      const result = await statefulEngine.evaluate('mixed', mixedRule, context2);

      expect(result.success).toBe(true); // Both conditions met
    });

    it('should preserve base engine functionality', async () => {
    ...
    it('should handle base engine errors gracefully', async () => {
      const result = await statefulEngine.evaluate('test', { invalidOp: ['value'] }, { value: 1 });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle empty contexts', async () => {
      const result = await statefulEngine.evaluate('test', { changed: ['value'] }, {});
      expect(result.success).toBe(false);
    });

    it('should handle complex nested contexts', async () => {
      ...
      await statefulEngine.evaluate('test', rule, context1);
      const result = await statefulEngine.evaluate('test', rule, context2);
  ...
  describe('Phase 1: State TTL and Expiration', () => {
    ...
    it('should start cleanup timer when stateExpirationMs is set', async () => {
    ...
    it('should not start cleanup timer when stateExpirationMs is null', async () => {
      const engine = new StatefulRuleEngine(baseEngine, {
    ...
    it('should track timestamps for each rule evaluation', async () => {
      const rule = { eq: ['status', 'active'] };
      const context = { status: 'active' };

      await statefulEngine.evaluate('rule1', rule, context);
    ...
    it('should update timestamp on each evaluation', async () => {
      const rule = { eq: ['status', 'active'] };
      const context = { status: 'active' };
    ...
    it('should clean up expired states', async () => {
      ...
      const rule = { eq: ['status', 'active'] };
      await engine.evaluate('rule1', rule, { status: 'active' });
    ...
    it('should not clean up non-expired states', async () => {
      const engine = new StatefulRuleEngine(baseEngine, {
    ...
    it('should stop and start cleanup timer', async () => {
      const engine = new StatefulRuleEngine(baseEngine, {
  ...
  describe('Phase 1: Deep Copy for Context Storage', () => {
    it('should deep copy context by default', async () => {
    ...
    it('should allow disabling deep copy', async () => {
      const engine = new StatefulRuleEngine(baseEngine, {
        enableDeepCopy: false,
      });
  ...
  describe('Phase 1: Listener Management', () => {
    it('should warn when listener count exceeds maxListeners', async () => {
    ...
    it('should get listener count for specific event', async () => {
      const callback1 = () => {};
      const callback2 = () => {};
    ...
    it('should get all listener counts', async () => {
      const callback = () => {};
  ...
  describe('Phase 1: State Statistics', () => {
    ...
    it('should calculate oldest state age', async () => {
      const rule = { eq: ['status', 'active'] };

      await statefulEngine.evaluate('rule1', rule, { status: 'active' });

      await new Promise((resolve) => setTimeout(resolve, 50));

      await statefulEngine.evaluate('rule2', rule, { status: 'active' });

      const oldestAge = statefulEngine.getOldestStateAge();
      expect(oldestAge).toBeGreaterThanOrEqual(50);
    });

    it('should return null for oldest state age when no states exist', async () => {
      const oldestAge = statefulEngine.getOldestStateAge();
  ...
  describe('Phase 1: Destroy and Cleanup', () => {
    it('should update clearState to handle timestamps', async () => {
      const rule = { eq: ['status', 'active'] };
    ...
    it('should destroy engine and cleanup all resources', async () => {
      const engine = new StatefulRuleEngine(baseEngine, {
        stateExpirationMs: 1000,
        storeHistory: true,
      });
